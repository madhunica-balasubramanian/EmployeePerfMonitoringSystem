"""fix enum column type for department.type

Revision ID: 75fe97abcdc3
Revises: 6bf34856970e
Create Date: 2025-04-18 20:48:47.181400
...
revision: str = '75fe97abcdc3'
down_revision: Union[str, None] = '6bf34856970e'
...
def upgrade() -> None:
    ...
    op.alter_column('departments', 'type', 
                    type_=postgresql.ENUM('USPS', 'HEALTHCARE', 'TRANSPORTATION', name='department_type_enum'),
                    existing_type=sa.String(14))
    # Update role type enum
    op.alter_column('users', 'role',
                    type_=postgresql.ENUM('ADMIN', 'SUPERVISOR', 'EMPLOYEE', 
                                        name='role_type_enum'),
                    existing_type=sa.String(50))
    
    # Update department role enum
    op.alter_column('users', 'department_role',
                    type_=postgresql.ENUM('HEALTHCARE_SUPERVISOR', 'HEALTHCARE_ADMIN', 
                                        'HEALTHCARE_NURSE', 'USPS_SUPERVISOR', 
                                        'USPS_MAIL_CARRIER', 'USPS_OFFICE_ADMIN',
...
def downgrade() -> None:
    ...
    sa.Column('department_role', postgresql.ENUM('HEALTHCARE_STAFF', 'HEALTHCARE_STAFF2', 'HEALTHCARE_STAFF3', 'USPS_STAFF2', 'USPS_STAFF3', name='department_role_enum'), autoincrement=False, nullable=False),
    sa.Column('department_id', sa.INTEGER(), autoincrement=False, nullable=True),
    ...
    op.alter_column('departments', 'type', 
                    type_=sa.String(14),
                    existing_type=postgresql.ENUM('USPS', 'HEALTHCARE', 'TRANSPORTATION', name='department_type_enum'))
    # Revert department role to string
    op.alter_column('users', 'department_role',
                    type_=sa.String(50),
                    existing_type=postgresql.ENUM('HEALTHCARE_SUPERVISOR', 'HEALTHCARE_ADMIN', 
                                                'HEALTHCARE_NURSE', 'USPS_SUPERVISOR', 
                                                'USPS_MAIL_CARRIER', 'USPS_OFFICE_ADMIN',
                                                name='department_role_enum'))
    
    # Revert role type to string
    op.alter_column('users', 'role',
                    type_=sa.String(50),
                    existing_type=postgresql.ENUM('ADMIN', 'SUPERVISOR', 'EMPLOYEE',
                                                name='role_type_enum'))
    # ### end Alembic commands ###
