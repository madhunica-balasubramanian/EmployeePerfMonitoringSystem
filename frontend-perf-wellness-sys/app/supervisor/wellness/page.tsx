...
export default function SupervisorWellnessPage() {
  ...
  const [textValueMapping, setTextValueMapping] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        ...
        // Check if the selected metric is text-based
        const textBased = response.some(employee => 
          employee.metrics.some(m => 
            m.metric_name === (selectedMetric || allMetricNames[0]) && 
            m.value_text !== null && 
            m.value_text !== ''
          )
        );
        
        setIsTextBased(textBased);
        
        // Process metrics data for visualization
        const { data, mapping } = processMetricsData(response, selectedMetric || allMetricNames[0]);
        setMetrics(data);
        setTextValueMapping(mapping || {});
      } catch (error) {
  ...
  const processMetricsData = (data: Employee[], metricName: string) => {
    ...
    // Check if the selected metric is text-based or numerical
    const isTextBased = data.some(employee => 
      employee.metrics.some(m => 
        m.metric_name === metricName && m.value_text !== null && m.value_text !== ''
      )
    );
    
    if (isTextBased) {
      // For text-based metrics, create a different data structure
      // Map text values to numerical scores for visualization
      return processTextBasedMetrics(data, metricName, allDates);
    } else {
      // For numerical metrics, use the original approach
      return {
        data: processNumericalMetrics(data, metricName, allDates),
        mapping: {}
      };
    }
  };
  ...
  const processNumericalMetrics = (data: Employee[], metricName: string, allDates: string[]) => {
  ...
  // Process text-based metrics by mapping them to numerical values
  const processTextBasedMetrics = (data: Employee[], metricName: string, allDates: string[]) => {
    // First, collect all unique text values for this metric
    const uniqueTextValues = new Set<string>();
    data.forEach(employee => {
      employee.metrics
        .filter(m => m.metric_name === metricName && m.value_text)
        .forEach(m => {
          if (m.value_text) uniqueTextValues.add(m.value_text);
        });
    });
    
    // Create a mapping from text values to numerical scores
    const textToNumericMap: Record<string, number> = {};
    
    // Common mappings for different metric types
    ...
    const agreementMapping: Record<string, number> = {
      'Strongly Agree': 5,
      'Agree': 4,
      'Neutral': 3,
      'Disagree': 2,
    ...
    const wellnessMapping: Record<string, number> = {
      'Excellent': 5,
      'Good': 4,
      'Fair': 3,
      'Poor': 2,
    ...
    // Determine which mapping to use based on the metric name
    let selectedMapping: Record<string, number> = {};
    if (metricName.toLowerCase().includes('satisfaction')) {
      selectedMapping = satisfactionMapping;
    } else if (metricName.toLowerCase().includes('agree')) {
      selectedMapping = agreementMapping;
    } else {
      selectedMapping = wellnessMapping;
    }
    
    // Apply the mapping to our unique text values
    uniqueTextValues.forEach(value => {
      if (selectedMapping[value]) {
        textToNumericMap[value] = selectedMapping[value];
      } else {
        // If no mapping exists, assign sequential values
        const values = Array.from(uniqueTextValues);
        values.sort(); // Sort alphabetically
        values.forEach((val, index) => {
          textToNumericMap[val] = values.length - index; // Highest value first
        });
      }
    });
    
    // Now process the data using our text-to-numeric mapping
    const processedData = allDates.map(date => {
      ...
      data.forEach((employee) => {
        ...
        if (metric && metric.value_text) {
          dailyData[employeeName] = textToNumericMap[metric.value_text] || 0;
        } else if (metric && metric.value_numeric !== null) {
          dailyData[employeeName] = metric.value_numeric;
        } else {
          dailyData[employeeName] = 0;
        }
      });
    ...
    return { data: processedData, mapping: textToNumericMap };
  };
              ...
              <>
                <div className="mb-6">
                  <p className="text-gray-600">
                    This chart shows {selectedMetric} metrics for each employee over time.
                    {isTextBased ? " Text values have been converted to numerical scores for visualization." : ""}
                  </p>
                  
                  {/* If text-based, show the mapping legend */}
                  {isTextBased && Object.keys(textValueMapping).length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-3">
                      <span className="font-medium">Value Mapping:</span>
                      {Object.entries(textValueMapping).map(([text, value]) => (
                        <span key={text} className="px-2 py-1 bg-gray-100 rounded-md text-sm">
                          {text}: {value}
                        </span>
                      ))}
                    </div>
